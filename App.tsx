...
import { AppView, User, UserRank, LoanRecord, Notification, LogEntry } from './types';
...
import { User as UserIcon, Home, Briefcase, Medal, LayoutGrid, Users, Wallet, AlertTriangle, Shield } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async (retries = 3) => {
      try {
        ...
        // Fetch logs separately or include in data
  ...
  const logAction = async (action: string, userName: string = 'SYSTEM') => {
    try {
      const logData = {
        id: `LOG-${Date.now()}`,
        user: userName,
  ...
  const handleLogin = (phone: string, password?: string) => {
    ...
    if (isAdmin) {
      ...
      setUser(adminUser);
      setCurrentView(AppView.ADMIN_DASHBOARD);
      logAction('Đăng nhập hệ thống quản trị', 'ADMIN');
      return;
    }
    ...
    if (existingUser) {
      ...
      logAction('Đăng nhập ứng dụng', existingUser.fullName);
    } else {
  ...
  const handleRegister = async (userData: Partial<User>) => {
    ...
    setUser(newUser);
    setCurrentView(AppView.DASHBOARD);
    logAction('Đăng ký tài khoản mới', newUser.fullName);
  };
  ...
  const handleApplyLoan = (amount: number, signature?: string) => {
    ...
    logAction(`Yêu cầu vay ${amount.toLocaleString()} đ`, user.fullName);
  ...
  const handleAdminLoanAction = (loanId: string, action: 'APPROVE' | 'DISBURSE' | 'SETTLE' | 'REJECT', reason?: string) => {
    ...
    const actionMap: Record<string, string> = {
      ...
      'SETTLE': 'Tất toán khoản vay',
      'REJECT': 'Từ chối khoản vay'
    };
    logAction(`${actionMap[action]} ID: ${loanId}`, 'ADMIN');
  ...
  const handleAdminUserAction = (userId: string, action: 'APPROVE_RANK' | 'REJECT_RANK') => {
    if (action === 'APPROVE_RANK') {
      ...
      if (targetUser && targetUser.pendingUpgradeRank) {
        logAction(`Duyệt nâng hạng ${targetUser.pendingUpgradeRank} cho ${targetUser.fullName}`, 'ADMIN');
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case AppView.ADMIN_LOGS: return <AdminLogs logs={logs} onBack={() => setCurrentView(AppView.ADMIN_DASHBOARD)} />;
          ...
          <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-[#111111]/95 backdrop-blur-xl border-t border-white/10 px-4 py-4 flex justify-between items-center z-[50] safe-area-bottom">
            {user?.isAdmin ? (
              <>
                ...
                <button onClick={() => setCurrentView(AppView.ADMIN_LOGS)} className={`flex flex-col items-center gap-1 flex-1 ${currentView === AppView.ADMIN_LOGS ? 'text-[#ff8c00]' : 'text-gray-500'}`}><Shield size={22} /><span className="text-[7px] font-black uppercase tracking-widest">Nhật ký</span></button>
